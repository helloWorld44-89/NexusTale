...
const updateProseFingerpint = `-- name: UpdateProseFingerpint :exec
UPDATE projects SET prose_fingerprint = $2, updated_at = now() WHERE id = $1
`

type UpdateProseFingerpintParams struct {
	ID               uuid.UUID `json:"id"`
	ProseFingerprint []byte    `json:"prose_fingerprint"`
}

func (q *Queries) UpdateProseFingerpint(ctx context.Context, arg UpdateProseFingerpintParams) error {
	_, err := q.db.Exec(ctx, updateProseFingerpint, arg.ID, arg.ProseFingerprint)
	return err
}
